...
<div data-theme=\"a\" data-role=\"header\">
		 ...
		 <a data-role=\"button\" data-direction=\"reverse\" data-transition=\"flip\" href=\"notekle.php\" data-icon=\"flat-plus\" data-iconpos=\"left\" id=\"notekle\"class=\"ui-btn-right\">Not Ekle</a>
...
<!--/AÇILIR PANEL -->
			...
			if ($gunn <= 30 ){
				if ($gunn==0){
					echo "Gün içinde {$geoplugin->city} yakınlarından giriş yapıldı.";
				}
				else{
					echo "".$gunn." gün önce {$geoplugin->city} yakınlarından giriş yapıldı";
				}
				
			}
			else if (($gunn >= 30) and ($gunn <= 360)){
				echo "".floor($gunn/30)." ay önce {$geoplugin->city} yakınlarından giriş yapıldı";
			}
			else if ($gunn >= 360){
				echo "".floor($gunn/360)." yıl önce {$geoplugin->city} yakınlarından giriş yapıldı";
			}

			echo "<br>Son giriş yapılan IP: {$geoplugin->ip} <br />";
